...
The game was simple. You get a string consisting of a number between two letters. Depending on whether the letter was in front of the number or after
...
But the game became too easy for Nakov really quick. He decided to complicate it a bit by doing the same but with multiple strings keeping track
of only the total sum of all results. Once he started to solve this with more strings and bigger numbers it became quite hard to do it only in his mind.
...
A is Uppercase and its position in the alphabet is 1. So we divide the number 12 with the position 1 (12/1 = 12). Then we move to the letter after
the number. b is lowercase and its position is 2. So we add 2 to the resulted number (12+2=14). Similarly for the second string s is lowercase and
its position is 19 so we multiply it with the number (17*19 = 323). Then we have Uppercase G with position 7, so we subtract it from the resulted number
...
for i in range(len(command)):
   ...
   first_letter = command[i][0]
   last_letter = command[i][-1]
   number = float(command[i][1:-1])

   if  65 <= ord(first_letter) <= 90:
       first_letter = 1 / (ord(first_letter)-64)
   elif 97 <= ord(first_letter) <=122:
        first_letter = ord(first_letter) - 96
   #print(first_letter)

   if 65 <= ord(last_letter) <= 90:
      last_letter = - ord(last_letter) + 64
   elif 97 <= ord(last_letter) <=122:
      last_letter = ord(last_letter) - 96

   total_sum += (number*first_letter + last_letter)
   #print(total_sum)
